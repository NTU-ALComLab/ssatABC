...
void
Pb_WriteWMC( Abc_Ntk_t * pNtk , char * name , int numPo , int fModel )
{
   FILE * out;
	int  numVar , numCla;
	...
	numCla = 3 * Abc_NtkNodeNum( pNtk ) + 2 + 1; // 2 for Po connection , 1 for Po assertion
...
void
Pb_WriteWMCCla( FILE * out , Abc_Ntk_t * pNtk , int numPo )
{
...
void 
Pb_WriteWMCWeight( FILE * out , Abc_Ntk_t * pNtk , int numPo , int fModel )
{
   Abc_Obj_t * pObj;
	int i;
   
	if ( fModel ) {
      if ( !pNtk->pModel ) printf("  > [Error] Model does not exist! (output is invalid)\n");
		else {
