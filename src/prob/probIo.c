...
void
Pb_GenProbNtk( Abc_Ntk_t * pNtk , float error , float defect )
{
...
void
Pb_CountErrorGates( Abc_Ntk_t * pNtk )
{
	Abc_Obj_t * pObj;
	float ratio;
	int numErr , i;
...
void
Pb_WritePBN( Abc_Ntk_t * pNtk , char * name )
{
   FILE * out;
...
void
Pb_WritePBNPio( FILE * out , Abc_Ntk_t * pNtk )
{
	Abc_Obj_t * pObj;
	int i;
...
void
Pb_WritePBNGate( FILE * out , Abc_Ntk_t * pNtk )
{
	Abc_Obj_t * pObj;
	int i;
	...
	Abc_NtkForEachPo( pNtk , pObj , i )
	{
		fprintf( out , ".names %d %s\n" 
				       , Abc_ObjId  ( Abc_ObjFanin0( pObj ) )
				       , Abc_ObjName( pObj ) );
      if ( !Abc_ObjFaninC0(pObj) ) fprintf( out , "1 1\n" );
		else                         fprintf( out , "0 1\n" );
	}
}
